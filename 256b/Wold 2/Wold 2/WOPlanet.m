...
- (void) determineShape
{
    ...
    c2 = 0.06 * rand() / RAND_MAX;
    c3 = 0.04 * rand() / RAND_MAX;
    c4 = 0.02 * rand() / RAND_MAX;
...
- (CGColorRef) generateSquaresPatternFromColorComponents:(float*)components
{
    ...
    for (int y = 0; y < h / squareDim; y++) {
        for (int x = 0; x < w / squareDim; x++) {
            ...
            //float squareColor[4] = {fill[0] + offset, fill[1] + offset, fill[2] + offset, 1.0};
            square = CGRectMake(x * squareDim, y * squareDim, squareDim, squareDim);
            //CGContextSetFillColor(bitmapContext, fill);//squareColor);
...
- (void) setupLayer
{
    ...
    rotationAnimation.repeatCount = 9999999999999999; 
