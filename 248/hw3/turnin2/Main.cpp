...
void setTextures(Shader *shader, aiScene const * scene, aiMesh const * mesh) {
    ...
    if (shader == envMapShader)
    {
        ...
        GLuint env = glGetUniformLocation(shader->programID(), "cubeMap");
...
void generateCubeMap()
{
    ...
    GL_CHECK(glGenTextures(1, &cubeMapTextureID));
    GL_CHECK(glBindTexture(GL_TEXTURE_CUBE_MAP, cubeMapTextureID));
    glTexParameterf(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
    glTexParameterf(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
    glTexParameterf(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
    glTexParameterf(GL_TEXTURE_CUBE_MAP, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
...
//        transNode(cathedralScene, cathedralScene->mRootNode);  // TODO: STILL NEED TO FIGURE OUT WHY I DO THIS
        ...
        lightPos.y = lightDistance * sinf(lightPitch);
        lightPos.z = lightDistance * sinf(lightYaw) * cosf(lightPitch);
        GLfloat light0_position[] = { lightPos.x, lightPos.y, lightPos.z, 0 };
        glLightfv( GL_LIGHT0, GL_POSITION, light0_position );
        ...
        GL_CHECK(glTexImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + face, 0, GL_RGBA,
                     CUBE_MAP_SIZE, CUBE_MAP_SIZE, 0, GL_RGBA, GL_FLOAT, NULL));
        GL_CHECK(glCopyTexSubImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X + face, 0, 0, 0, 0, 0, CUBE_MAP_SIZE, CUBE_MAP_SIZE));
