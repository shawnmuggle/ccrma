...
void Compressor::processReplacing (float** inputs, float** outputs, VstInt32 sampleFrames)
{
	...
	for (int i = 0; i < sampleFrames; i++)
	{
        ...
        // TODO: REPLACE release_amount with a general amount
        filtstate = (1.0 - release_amount) * filtstate + release_amount * pow(input, 2.0);

        double sqrt_filtstate = sqrt(filtstate);
        
		/////////////  GAIN COMPUTER  ///////////////////
		// take signal level estimate into log domain
		if (sqrt_filtstate<0.00001)				// if below -100dB, set to -100dB
			loglev=20.0*log10(0.00001);		// to prevent taking log of zero
		else
            loglev=20.0 * log10(sqrt_filtstate);
        ...
        if (loglev >= logthresh)
        {
            dbgainval = 20 * log10(pow(sqrt_filtstate / threshold, 1 / comp_ratio - 1));
        }
