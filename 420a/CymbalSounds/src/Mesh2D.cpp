...
namespace stk {
    ...
    Mesh2D :: Mesh2D( short nX, short nY )
    {
        ...
        short i;
        for (i=0; i<NYMAX; i++) {
            filterY_[i].setGain( 1.0 );
            filterY_[i].setA1(0.0);
            filterY_[i].setA2(0.0);
        }
        
        for (i=0; i<NXMAX; i++) {
            filterX_[i].setGain( 1.0 );
            filterX_[i].setA1(0.0);
            filterX_[i].setA2(0.0);
        }
    ...
    void Mesh2D :: noteOn( StkFloat frequency, StkFloat amplitude, StkFloat brightness )
    {
        
        int radius = (1 - brightness) * NX_ / 2;

        printf("Radius is %d", radius);
        ...
        if ( counter_ & 1 ) {
            ...
            float dist;
            for (int x = xInput_ - radius; x < xInput_ + radius; x++) {
                if (x < 0 || x >= NX_) continue;
                for (int y = yInput_ - radius; y < yInput_ + radius; y++) {
                    if (y < 0 || y >= NY_) continue;
                    dist = sqrt(pow(xInput_ - x, 2) + pow(yInput_ - y, 2));
                    vxp1_[x][y] += cos(dist) * amplitude;
                    vyp1_[x][y] += cos(dist) * amplitude;
                }
            }
        } else {
            ...
            for (int x = xInput_ - radius; x < xInput_ + radius; x++) {
                ...
                for (int y = yInput_ - radius; y < yInput_ + radius; y++) {
                    if (y < 0 || y >= NY_) continue;
                    dist = sqrt(pow(xInput_ - x, 2) + pow(yInput_ - y, 2));
                    vxp_[x][y] += cos(dist) * amplitude;
                    vyp_[x][y] += cos(dist) * amplitude;
                }
            }            
        }

            // NOTE(mrotondo): I think this breaks "replayability": calling noteOn after the mesh has been ringing might put the delays into inconsistent states?
//            vxp1_[xInput_][yInput_].tick(amplitude); 
//            vyp1_[xInput_][yInput_].tick(amplitude);
    ...
    StkFloat Mesh2D :: tick0( void )
    {
        ...
        for (y=0; y<NY_-1; y++) {
            StkFloat filtered = filterY_[y].tick(vxm_[0][y]);
            vxp1_[0][y] = filtered;
            vxm1_[NX_-1][y] = vxp_[NX_-1][y];
        }
        for (x=0; x<NX_-1; x++) {
            StkFloat filtered = filterX_[x].tick(vym_[x][0]);
            vyp1_[x][0] = filtered;
